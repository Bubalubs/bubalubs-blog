<script setup>
    ...
    useHead({
        ...
        meta: [
            {
                name: 'description',
                content: 'A guide to using Stripe 3DSecure Checkout using SetupIntent with Laravel and Vue.js',
            }
        ],
        link: [
            {
...
<template>
    <main>
        ...
        <p>Use payment intents if you want the customer to be instantly charged within the checkout process. You can use setup intents to do a 3DSecure check on the card and then save it for charging later (Or practically straight away) in your back end code. If using payment intents you have to use Stripe's webhooks to listen for the payment succeeding and then handle and fullfill the order.</p>

        <p>In these examples I will use the SetupIntent approach and charge the customer after the checkout process.</p>

        <h2>Create SetupIntent on your server</h2>

        <p>First create a SetupIntent on your server and send it to your front-end. This token can only be used once so is idempotent and stops any duplication happening.</p>
        ...
        <h2>Create or retrieve customer and then attach payment method to customer on your server</h2>
