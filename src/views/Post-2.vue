<script setup>
    ...
    useHead({
        ...
        meta: [
            {
                name: 'description',
                content: 'How to Fix LetsEncrypt to work without Laravel Forge on previously provisioned server',
            }
        ],
        link: [
            {
...
<template>
    <main>
        <h1>Fix LetsEncrypt to work without Laravel Forge on previously provisioned server</h1>

        <p class="mb-12">April 27, 2020</p>

        <p>If you provisioned a server using Laravel forge and are no longer relying on that for TLS certificate renewals, if the renewal process stops working you can do the following steps to fix it using the LetsEncrypt CLI tool and adjusting your site nginx config.</p>

        <p>First SSH into your server and run the following command which will create a new certificate for your site without it trying to edit the nginx config automatically.</p>
        ...
        <p>When running the command you will see a list of your sites that certificates can be created for. Select your site by typing in it's number.</p>

        <p>Then edit your site's nginx config file:</p>

        <highlightjs language="bash" :code="example3" />

        <p>Then remove all the existing Forge SSL settings and add the following instead (replacing YOURDOMAIN with your own domain):</p>
        ...
        <p>Then your certificate should be working correctly and should get autorenewed by the cerbot.</p>
